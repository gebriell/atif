...
class DatasetCredit(AbstractDataset):
    ...
    def load(self):
        ...
        # data = norm_data._append(fraud_data)
        data = pd.concat([norm_data, fraud_data], ignore_index=True)  # Updated line
        ...
        # Setting the valid and fraud attributes
        self.fraud = len(fraud_data)
        self.valid = len(norm_data)

        # count_class = pd.value_counts(data['Class'])
        count_class = pd.Series(data['Class']).value_counts()  # Updated line
        count_class.plot(kind='bar', rot=0)
        plt.title("Class Distribution")
        plt.xticks(range(2), ["Normal", "Fraud"])
        plt.xlabel("Class")
        plt.ylabel("Frequency")
        plt.savefig("/home/dev/files/data/env/Attention-based-isolation-forest/atif/credit_dataset.jpg")
